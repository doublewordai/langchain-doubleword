...
Figures are from the async (high-throughput) tier; the 24-hour batch tier is cheaper still.
...
- Set `APP_MODEL` and `JUDGE_MODEL` to grade a model of your choice from the model catalog with a smarter judge model. 
- Pick the tier with `--tier`: `batch` (24h, cheapest, the default), `async` (high-throughput), or
...
Each run creates an experiment. Open it for:
...
The authoritative spend lives with Doubleword:
